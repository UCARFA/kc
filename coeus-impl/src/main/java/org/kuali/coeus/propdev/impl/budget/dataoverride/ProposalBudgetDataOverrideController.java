...
public class ProposalBudgetDataOverrideController extends ProposalBudgetControllerBase {
    ...
    public ModelAndView createOverride(@ModelAttribute("KualiForm") ProposalBudgetForm form)
        ...
        int changeNumber = budget.getBudgetParent().getDocument().getDocumentNextValue(CHANGE_NUMBER);
        ...
        //under certain cases prePersist is not being called by JPA.  Manually calling it here.
        ...
        if(getKualiRuleService().applyRules(new BudgetDataOverrideEvent((ProposalDevelopmentDocument) budget.getBudgetParent().getDocument(), newBudgetChangedData))){
            //special budget justification handling
            ...
            final List<BudgetChangedData> budgetChangedDataList= new ArrayList<>();
            ...
            form.getDevelopmentProposal().getBudgetChangedDataList().add(getDataObjectService().save(newBudgetChangedData));

            getDataObjectService().save(budget.getBudgetParent().getDocument().getDocumentNextvalueBo(CHANGE_NUMBER));
