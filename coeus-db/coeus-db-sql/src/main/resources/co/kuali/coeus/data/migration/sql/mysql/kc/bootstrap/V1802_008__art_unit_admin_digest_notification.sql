...
values ((select max(id) from seq_notification_type_id), 1, '-2', 'Award Report Tracking Unit Overdue Digest Notification', 'Overdue Reports',
...
values ((select max(id) from seq_notification_type_id), (select notification_type_id from notification_type where module_code = 1 and action_code = '-2'), 'KC-AWARD:All Unit Administrators', 'admin', now(), 1, uuid(), '3');
...
values ((select max(id) from seq_ntfctn_mdl_role_qlfr_id), (select notification_module_role_id from notification_module_role where role_name = 'KC-AWARD:All Unit Administrators'), uuid(), 1, now(), 'admin', 'subunits');
