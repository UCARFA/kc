...
values ((SELECT (MAX(ID)) FROM SEQ_QUESTIONNAIRE_REF_ID), (SELECT (MAX(ID)) FROM SEQ_QUESTIONNAIRE_ID),1,
'ED524 Budget V-1.3 questions','These questions support  ED524 Budget V-1.3 forms.',
NOW(), 'admin','Y',1,UUID(),null);
...
((SELECT (MAX(ID)) FROM SEQ_QUESTIONNAIRE_REF_ID),10120,1,'C','Do the instructions for this project indicate this is a restricted program?','I',4,1,null,null,null,1,1,NOW(),
 'admin',1,UUID(),null);
...
VALUES ((SELECT (MAX(ID)) FROM  SEQ_QUESTIONNAIRE_REF_ID),(SELECT QUESTIONNAIRE_REF_ID FROM QUESTIONNAIRE WHERE NAME = 'ED524 Budget V-1.3 questions' AND SEQUENCE_NUMBER = 1),
...
'ED524 Budget V-1.3 questionnaire',NOW(),'admin',1,UUID(),'N');
...
VALUES ((select max(id) from SEQ_QUESTIONNAIRE_REF_ID), 'http://apply.grants.gov/forms/ED_524_Budget_1_3-V1.3', 'ED_524_Budget_1_3-V1.3',
(select questionnaire_id from questionnaire where name='ED524 Budget V-1.3 questions'), NOW(), 'admin', UUID(), '1');
