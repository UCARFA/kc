...
VALUES ((SELECT (MAX(ID))
         FROM SEQ_QUESTIONNAIRE_REF_ID), -5, 1, 'Clinical Trials Decision Tree', 'Relates to clinical trials.', NOW(), 'admin', 'Y', NULL, 1, UUID(),
        NULL, NULL);
...
VALUES
  ((SELECT (MAX(ID))
    FROM SEQ_QUESTIONNAIRE_REF_ID), -10131, 1, 'C', 'Are participants assigned to an intervention?', 'I', 4, 1, NULL, NULL, NULL, 1, 1, NOW(),
   'admin', 1, UUID(), NULL);
...
VALUES
  ((SELECT (MAX(ID))
    FROM SEQ_QUESTIONNAIRE_REF_ID), -10132, 1, 'C', 'Is the study designed to evaluate the effect of the intervention on the participants?', 'I', 4,
                                    1, NULL, NULL, NULL, 1, 1, NOW(), 'admin', 1, UUID(), NULL);
...
VALUES
  ((SELECT (MAX(ID))
    FROM SEQ_QUESTIONNAIRE_REF_ID), -10133, 1, 'C', 'Is the effect being evaluated on health-related biomedical or behavioral outcome?', 'I', 4, 1,
...
VALUES ((SELECT (MAX(ID))
         FROM SEQ_QUESTIONNAIRE_REF_ID), 3, 0, (SELECT QUESTIONNAIRE_REF_ID
                                                FROM QUESTIONNAIRE
                                                WHERE QUESTIONNAIRE_ID = -5 AND SEQUENCE_NUMBER = 1), 1, 'RES-BOOT10000',
                                         'Clinical Trials Decision Tree', NOW(), 'admin', 1, UUID(), 'N');
