...
values ((select max(id) from seq_s2s_error_id),
        ...
        'Agency division code must be specified', 'proposal.SponsorProgramInformation', now(), 'admin', 1, uuid());

insert into seq_s2s_error_id values (null);
insert into s2s_error (s2s_error_id, message_key, message, fix_link, update_timestamp, update_user, ver_nbr, obj_id)
values ((select max(id) from seq_s2s_error_id),
        '/GrantApplication/Forms/NSF_CoverPage_1_8/NSFUnitConsideration/ProgramCode',
        'Agency program code must be specified', 'proposal.SponsorProgramInformation', now(), 'admin', 1, uuid());
