...
<c:set var="projectPersonnel" value="${KualiForm.awardCreditSplitBean.personsSelectedForCreditSplit}" />
...
<c:choose>
    <c:when test="${fn:length(KualiForm.awardCreditSplitBean.personsSelectedForCreditSplit) != 0}">
    ...
    <kul:innerTab tabTitle="Combined Credit Split" parentTab="Project Personnel" defaultOpen="true"
        ...
        <%-- Heading with InvestigatorCreditType Description --%>
            ...
            <c:forEach items="${projectPersonnel}" var="projectPerson" varStatus="ppStatus">
              ...
              <c:set var="projectPersonProperty" value="document.awardList[0].personsSelectedForCreditSplit[${ppStatus.index}]" />
